...
import com.teamide.app.generater.code.sql.CustomGenerater;
import com.teamide.app.generater.code.sql.DeleteGenerater;
import com.teamide.app.generater.code.sql.InsertGenerater;
import com.teamide.app.generater.code.sql.SelectGenerater;
import com.teamide.app.generater.code.sql.SqlGenerater;
import com.teamide.app.generater.code.sql.UpdateGenerater;
...
public abstract class SQLDaoGenerater extends BaseDaoGenerater {
	...
	public SQLDaoGenerater(DaoBean dao, File sourceFolder, AppBean app, AppContext context) {
	...
	public void buildSQLData() {
		...
		if (sqlProcess.getSqlType().indexOf("SELECT") >= 0) {
			...
			sqlGenerater = new SelectGenerater(context, sqlProcess.getSelect());
			...
			if (sqlProcess.getSelect().getSubselects() != null && sqlProcess.getSelect().getSubselects().size() > 0) {
				...
				for (SubSelect subSelect : sqlProcess.getSelect().getSubselects()) {
					if (subSelect.getSelect() != null) {
						...
						sqlGenerater = new SelectGenerater(context, subSelect.getSelect());
						sqlGenerater.generate(2);
			...
			sqlGenerater = new InsertGenerater(context, sqlProcess.getInsert());
			sqlGenerater.generate(2);
			data.put("$content", sqlGenerater.getContent());
			...
			sqlGenerater = new UpdateGenerater(context, sqlProcess.getUpdate());
			sqlGenerater.generate(2);
			data.put("$content", sqlGenerater.getContent());
			...
			sqlGenerater = new DeleteGenerater(context, sqlProcess.getDelete());
			sqlGenerater.generate(2);
			data.put("$content", sqlGenerater.getContent());
			...
			sqlGenerater = new CustomGenerater(context, sqlProcess.getCustomSql());
			sqlGenerater.generate(2);
			data.put("$content", sqlGenerater.getContent());
		...
		if (sqlGenerater != null) {
			...
			VariableGenerater variableGenerater = new VariableGenerater("data", "param");
			StringBuffer $variable_content = variableGenerater.generate(2, sqlGenerater.getVariablesForParam());
			if (StringUtil.isEmpty($variable_content)) {
				$variable_content = null;
			}
			data.put("$variable_content", $variable_content);

			variableGenerater = new VariableGenerater("data", "data");
			$variable_content = variableGenerater.generate(2, sqlGenerater.getVariablesForMapper());
			if (StringUtil.isEmpty($variable_content)) {
				$variable_content = null;
			}
			data.put("$variable_content_mapper", $variable_content);

			ValidateGenerater validateGenerater = new ValidateGenerater("data", "data");
			StringBuffer $validate_content = validateGenerater.generate(2, sqlGenerater.getValidatesForMapper());
			if (StringUtil.isEmpty($validate_content)) {
				$validate_content = null;
			}
			data.put("$validate_content_mapper", $validate_content);
		}
