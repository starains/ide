...
(function() {
	...
	Editor.prototype.initOptions = function(options) {
		...
		this.model = $.extend(true, {}, this.model_original);
		this.historys = [];
		this.data = {
			view : '',
			model : this.model,
	...
	Editor.prototype.viewDesign = function() {
		...
		if (this.type) {
			...
			if (that.lastText && that.lastText == data.text) {
				...
				coos.POST(_SERVER_URL + "/api/data/toModel", data, function(res) {
					if (res.errcode == 0) {
						if (changed) {
							that.recordHistory();
						}
	...
	Editor.prototype.changeModel = function(callBuild) {
		...
		coos.trimKey(this.model, [ undefined, null, "" ], true);
		var data = {
			json : JSON.stringify(this.model),
			type : this.type,
			filename : this.file.name
		}
		coos.POST(_SERVER_URL + "/api/data/toText", data, "text", function(res) {
			that.ace.setValue(res);
			that.ace.clearSelection();
		});
	};


	Editor.prototype.changeCode = function(content) {
		if (coos.isEmpty(content) && coos.isEmpty(this.tab.content)) {
			this.tab.changed = false;
		} else if (content == this.tab.content) {
			this.tab.changed = false;
		} else {
			this.tab.changed = true;
		}
	};
...
(function() {
	...
	Editor.prototype.toSave = function() {
		this.options.onSave && this.options.onSave(this.ace.getValue());
	};
	...
	Editor.prototype.getHelpHtml = function() {
		...
		var $info = $('<div class="pdlr-10"/>');
		$info.append('<h4 class="coos-orange">说明</h4>');
		$info.append('<div class="coos-grey">1：数据格式以传输的报文为基础</div>');
		$info.append('<div class="coos-grey pdl-20">例如：传输{key1:value1,key2:value2}</div>');
		$info.append('<div class="coos-grey pdl-20">则映射变量：key1=value1,key2=value2。</div>');
		$info.append('<div class="coos-grey">2：内置变量映射：</div>');
		$info.append('<div class="coos-grey pdl-20">$data：为传输的数据</div>');
		$info.append('<div class="coos-grey pdl-20">$body：为requestBody</div>');
		$info.append('<div class="coos-grey pdl-20">$session：为AppSession</div>');
		$info.append('<div class="coos-grey pdl-20">$reqeust：为请求参数</div>');
		$info.append('<div class="coos-grey pdl-20">$header：为请求Header</div>');
		$info.append('<div class="coos-grey pdl-20">$user：为登录用户</div>');
		$info.append('<div class="coos-grey pdl-20">$cache：为AppSession缓存数据</div>');
		$info.append('<div class="coos-grey pdl-20">$result：为执行节点产生的结果，可以使用$result.节点名称</div>');
		$info.append('<div class="coos-grey">3：扩展函数：</div>');
		$info.append('<div class="coos-grey pdl-20">coos.now_date()：获取当前时间。示例：coos.now_date("yyyy-MM-dd HH:mm:ss"（非必填）)</div>');
		$info.append('<div class="coos-grey pdl-20">coos.generate_id()：生成ID。示例：coos.generate_id(12~32（非必填）)');
		$info.append('<div class="coos-grey pdl-20">coos.MD5()：MD5加密。示例：coos.MD5(value（必填）)');
		$info.append('<div class="coos-grey">4：使用说明：</div>');
		$info.append('<div class="coos-grey pdl-20">$user.user：可以取到登录用户ID</div>');
		return $info;
	};
	...
	Editor.prototype.getBeans = function(type) {
		var list = [];
		var app = this.project.app;
		if (app) {
			list = app.context[type] || [];
		}
		return list;
	};
...
(function() {
	...
	var html = '';
	html += '<div class="coos-nav coos-nav-full coos-bg-blue-grey coos-nav-full coos-nav-vertical coos-nav-xs">';
	html += '<template v-for="nav in navs">';
	html += '<li v-if="nav.line">';
	html += '</li>';
	html += '<li v-else v-bind:title="nav.text" v-on:click="nav.onClick()">';
	html += '<a :class="{\'coos-disabled\' : nav.disabled}">';
...
(function() {
	...
	var html = '';
	html += '<div class="coos-btn-group coos-btn-group-sm coos-bg-blue-grey">';
	html += '<a class="coos-btn" :class="{\'coos-active\' : view==\'code\'}" @click="viewCode">';
	html += '代码';
	html += '</a>';
	html += '<a class="coos-btn" :class="{\'coos-active\' : view==\'design\'}" @click="viewDesign">';
	html += '设计';
	html += '</a>';
	html += '</div>';
})();
(function() {
	...
	Editor.prototype.buildCode = function() {
		...
		var file = this.file;
		$code.empty();
		coos.plugin.load("ace", function() {
			coos.plugin.load("ext-language_tools", function() {
				var $pre = $("<pre ></pre>");
				$pre.css("width", "100%");
				$pre.css("height", "100%");
				$pre.text(tab.content);
				$code.append($pre);
				...
				if (file.type == "css") {
					...
					editor.getSession().setMode("ace/mode/gitignore");
...
(function() {
	...
	var setJexlScript = function(text, callback) {
		...
		$info.append('<div class="coos-grey pdl-20">$cache：为AppSession缓存数据</div>');
		$info.append('<div class="coos-grey pdl-20">$result：为执行节点产生的结果，可以使用$result.节点名称</div>');
		$info.append('<div class="coos-grey">3：使用说明：</div>');
		$info.append('<div class="coos-grey pdl-20">$user.user：可以取到登录用户ID</div>');

		var $form = $('<div class="coos-form coos-input-sm"/>');
		$html.append($form);

		$form.append('<textarea name="text" input-skin="0" skin-size="12" label="" class="coos-input" ></textarea>');


		var editBox = coos.box({
			title : 'JexlScript配置',
			openMove : true,
			html : $html,
			width : 500,
			...
			btns : [ {
				type : 'define',
				onClick : function() {
					var result = coos.form.validate($html);
					if (!result.valid) {
						coos.box.error(result.error.show);
					} else {
						var data = coos.form.getData($html);
						editBox.destroy();
...
(function() {
	...
	Editor.prototype.buildDesign = function() {
		...
		$design.empty();
		$design.append('<div class="coos-orange font-lg pd-80 text-center">暂未提供该文件格式在线设计，敬请期待！</div>');
	};
})();
(function() {
	...
	AppEditor.prototype.buildDesign = function() {
		...
		var $box = $('<div class="editor-app pd-10"></div>');
		$design.append($box);

		var model = this.model;


		var $html = $('<div class="coos-form coos-input-sm"/>');
		$html.append('<h3 class="pdlr-10">应用信息配置</h3>')
		$box.append($html);

		$html.append('<input name="name" required="1" input-skin="0" skin-size="12" label="名称" class="coos-input" />');

		$html.append('<input name="indexurl" input-skin="0" skin-size="6" label="首页地址" class="coos-input" />');
		$html.append('<input name="loginurl" input-skin="0" skin-size="6" label="登录地址" class="coos-input" />');

		$html.append('<input name="afterlogintourl" input-skin="0" skin-size="6" label="登录后跳转地址" class="coos-input" />');
		$html.append('<input name="afterlogouttourl" input-skin="0" skin-size="6" label="退出后跳转" class="coos-input" />');

		$html.append('<input name="ignoreurl" input-skin="0" skin-size="12" label="忽略地址" info="检查到匹配地址将直接放行，多个地址以“;”隔开如：/res/*;/abc/*。" class="coos-input" />');
		$html.append('<input name="loginignoreurl" input-skin="0" skin-size="12" label="登录忽略地址" info="检查到匹配地址将不检测是否登录，多个地址以“;”隔开如：/res/*;/abc/*。" class="coos-input" />');
		$html.append('<input name="requiredlogin" input-skin="0" skin-size="12" label="必须登录" class="coos-input coos-input-switch" />');
		$html.append('<input name="loginmatchurl" input-skin="0" skin-size="12" label="登录匹配地址" info="检查到匹配地址将需要登录，多个地址以“;”隔开如：/res/*;/abc/*。" class="coos-input" />');

		$html.append('<h3 class="pdlr-10 coos-orange">错误页面配置</h3>')
		$html.append('<input name="error_notfound" input-skin="0" skin-size="6" label="404地址" class="coos-input" />');
		$html.append('<input name="error_error" input-skin="0" skin-size="6" label="500地址" class="coos-input" />');
		$html.append('<input name="error_nopermission" input-skin="0" skin-size="6" label="无权限地址" class="coos-input" />');

		coos.format.init($html);
		model.error = model.error || {};
		for (let key in model.error) {
			model['error_' + key] = model.error[key];
		}
		coos.form.full($html, model, true);

		$html.find('.coos-input').change(function() {
			var name = $(this).attr('name');
			var value = $(this).val() || '';
			if (!coos.isEmpty(name)) {
				that.recordHistory();
...
(function() {
	...
	DatabaseEditor.prototype.buildDesign = function() {
		...
		var $html = $('<div class="coos-form coos-input-sm"/>');
		$box.append($html);
		$html.append('<h3>库信息配置</h3>')

		$html.append('<input name="driver" required="1" input-skin="0" skin-size="6" label="驱动" class="coos-input" />');
		$html.append('<input name="url" required="1" input-skin="0" skin-size="6" label="地址" class="coos-input" />');
		$html.append('<input name="username" input-skin="0" skin-size="6" label="用户名" class="coos-input" />');
		$html.append('<input name="password" input-skin="0" skin-size="6" label="密码" class="coos-input" />');
		$html.append('<input name="showsql" input-skin="0" skin-size="12" label="显示SQL" class="coos-input coos-input-switch" />');

		$html.append('<input name="initialsize" input-skin="0" skin-size="6" label="initialsize" class="coos-input" />');
		$html.append('<input name="maxtotal" input-skin="0" skin-size="6" label="maxtotal" class="coos-input" />');
		$html.append('<input name="minidle" input-skin="0" skin-size="6" label="minidle" class="coos-input" />');
		$html.append('<input name="maxidle" input-skin="0" skin-size="6" label="maxidle" class="coos-input" />');
		$html.append('<input name="maxwaitmillis" input-skin="0" skin-size="6" label="maxwaitmillis" class="coos-input" />');
		$html.append('<input name="maxactive" input-skin="0" skin-size="6" label="maxactive" class="coos-input" />');
		$html.append('<input name="maxwait" input-skin="0" skin-size="6" label="maxwait" class="coos-input" />');

		$html.append('<input name="validationquery" input-skin="0" skin-size="12" label="验证查询" info="输入一段SQL以此判断连接是否正常，如：SELECT 1" class="coos-input" />');

		$html.append('<input name="initializeclass" input-skin="0" skin-size="12" label="数据库初始化实现" info="需要实现com.coospro.app.ifaces.IDatabaseInitialize接口，可以修改Database对象中的属性值" class="coos-input" />');
...
(function() {
	...
	TableEditor.prototype.buildDesign = function() {
		...
		var $btn = $('<a class="mglr-10 coos-pointer coos-green">添加字段</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var column = {};
		...
		$(columns).each(function(index, column) {
			...
			if (coos.isTrue(column.primarykey)) {
				$li.append('<a class="mgl-10 coos-pointer coos-green updatePropertyBtn" property-type="primarykey"  >主键</a>');
			} else {
				$li.append('<a class="mgl-10 coos-pointer coos-orange updatePropertyBtn" property-type="primarykey" >非主键</a>');
			}
			if (coos.isTrue(column.nullable)) {
				$li.append('<a class="mgl-10 coos-pointer coos-green updatePropertyBtn" property-type="nullable"  >可空</a>');
			} else {
				$li.append('<a class="mgl-10 coos-pointer coos-orange updatePropertyBtn" property-type="nullable" >不可空</a>');
			}
			$li.find('.updatePropertyBtn').click(function() {
				var type = $(this).attr('property-type');
				if (!coos.isEmpty(type)) {
			...
			var $btn = $('<a class="mgl-10 coos-pointer coos-red">删除</a>');
			$li.append($btn);
			$btn.click(function() {
				that.recordHistory();
			...
			var $btn = $('<a class="mgl-10 coos-pointer coos-orange">上移</a>');
			$li.append($btn);
			$btn.click(function() {
			...
			var $btn = $('<a class="mgl-10 coos-pointer coos-orange">下移</a>');
			$li.append($btn);
			$btn.click(function() {
				var index = columns.indexOf(column);
...
(function() {
	...
	DaoEditor.prototype.toTest = function() {
		var that = this;
		var $html = $('<div class="coos-row"/>');
		$html.append('<h3 class="pdlr-10 coos-orange">请填写JSON格式数据作为参数</h3>');
		var $form = $('<div class="coos-form coos-input-sm"/>');
		$html.append($form);

		$form.append('<textarea name="body" input-skin="0" skin-size="12" label="参数" class="coos-input" />');

		$form.append('<textarea name="result" input-skin="0" skin-size="12" label="结果" class="coos-input" />');
				...
				onClick : function() {
					...
					if (!result.valid) {
						...
						that.options.onTest(data, function(res) {
							$html.find('[name="result"]').val(JSON.stringify(res));
						});
					}
				}
			} ]
...
(function() {
	...
	DaoEditor.prototype.createSqlSelectView = function(model) {
		...
		var $li = $('<li />');
		$ul.append($li);
		$li.append('<span class="pdr-10 coos-orange">SELECT</span>');

		var $btn = $('<a class="mgr-10 coos-pointer coos-green">添加查询字段</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var column = {};
			...
			column.name = "column";
			column.alias = 'name' + (columns.length);
			that.changeModel();
		});

		if (froms.length > 0) {
			var table = that.getTableByName(froms[0].table);
			if (table) {

				var $btn = that.createSelectColumnBtn(table.id, function(c) {
					that.recordHistory();
					var column = {};
					columns.push(column);
					column.tablealias = "T1";
		...
		$(columns).each(function(index, column) {
			...
			if (!coos.isTrue(column.custom)) {
				$li.append('<a class="mgl-10 coos-pointer coos-green updatePropertyBtn" property-type="custom"  >设为自定义</a>');
			} else {
				$li.append('<a class="mgl-10 coos-pointer coos-orange updatePropertyBtn" property-type="custom" >设为非自定义</a>');
			}
			if (coos.isEmpty(column.ifrule)) {
				$li.append('<a class="mgl-10 coos-pointer coos-green updatePropertyBtn" property-type="ifrule" property-value="1=1"  >设置条件</a>');
			} else {
				$li.append('<a class="mgl-10 coos-pointer coos-orange updatePropertyBtn" property-type="ifrule" property-value="">清空条件</a>');
			}
			var $btn = $('<a class="mgl-10 coos-pointer coos-orange">修改</a>');
			$li.append($btn);
			$btn.click(function() {});
			var $btn = $('<a class="mgl-10 coos-pointer coos-red">删除</a>');
			$li.append($btn);
		...
		if (subselects.length > 0) {
			$li.append('<span class="pdlr-10 coos-orange">子查询模型</span>');
		}
		var $btn = $('<a class="mgr-10 coos-pointer coos-green">添加子查询</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var subselect = {};
		...
		$(subselects).each(function(index, subselect) {
			...
			if (coos.isTrue(subselect.queryone)) {
				$li.append('<a class="mgl-10 coos-pointer coos-green updatePropertyBtn" property-type="queryone"  >查询单个</a>');
			} else {
				$li.append('<a class="mgl-10 coos-pointer coos-orange updatePropertyBtn" property-type="queryone" >查询列表</a>');
			}
			if (coos.isEmpty(subselect.ifrule)) {
				$li.append('<a class="mgl-10 coos-pointer coos-green updatePropertyBtn" property-type="ifrule" property-value="1=1"  >设置条件</a>');
			} else {
			...
			var $btn = $('<a class="mgl-10 coos-pointer coos-green">展开</a>');
			$li.append($btn);
		...
		if (froms.length > 0) {
			$li.append('<span class="pdr-10 coos-orange">FROM </span>');
		}
		var $btn = $('<a class="mgr-10 coos-pointer coos-green">添加FROM</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var from = {};
		...
		var $btn = $('<a class="mgr-10 coos-pointer coos-green">添加LEFT JOIN</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var leftjoin = {};
		...
		$(leftjoins).each(function(index, leftjoin) {
			var $li = $('<li class="pdl-10"/>');
			$ul.append($li);
			$li.append('<span class="pdr-10 coos-orange">LEFT JOIN</span>');
	...
	DaoEditor.prototype.appendGroupBy = function($ul, groups, froms) {
		...
		$ul.append($li);
		if (groups.length > 0) {
			$li.append('<span class="pdr-10 coos-orange">GROUP BY</span>');
		}
		var $btn = $('<a class="mgr-10 coos-pointer coos-green">添加分组</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var group = {};
	...
	DaoEditor.prototype.appendOrderBy = function($ul, orders, froms) {
		...
		$ul.append($li);
		if (orders.length > 0) {
			$li.append('<span class="pdr-10 coos-orange">ORDER BY</span>');
		}
		var $btn = $('<a class="mgr-10 coos-pointer coos-green">添加排序</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var order = {};
...
(function() {
	...
	DaoEditor.prototype.createSqlInsertView = function(model) {
		...
		var $li = $('<li />');
		$ul.append($li);
		$li.append('<span class="pdr-10 coos-orange">INSERT INTO</span>');
		...
		var $btn = $('<a class="mglr-10 coos-pointer coos-green">添加插入字段</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var column = {};
		...
		$(columns).each(function(index, column) {
			...
			if (!coos.isTrue(column.required)) {
				$li.append('<a class="mgl-10 coos-pointer coos-green updatePropertyBtn" property-type="required" >设为必填</a>');
			} else {
				$li.append('<a class="mgl-10 coos-pointer coos-orange updatePropertyBtn" property-type="required" >设为非必填</a>');
			}
			var $btn = $('<a class="mgl-10 coos-pointer coos-orange">修改</a>');
			$li.append($btn);
			$btn.click(function() {});
...
(function() {
	...
	DaoEditor.prototype.createSqlUpdateView = function(model) {
		...
		var $li = $('<li />');
		$ul.append($li);
		$li.append('<span class="pdr-10 coos-orange">UPDATE</span>');
		...
		var $btn = $('<a class="mglr-10 coos-pointer coos-green">添加更新字段</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var column = {};
...
(function() {
	...
	DaoEditor.prototype.createSqlDeleteView = function(model) {
		...
		var $li = $('<li />');
		$ul.append($li);
		$li.append('<span class="pdr-10 coos-orange">DELETE FROM</span>');
...
(function() {
	...
	DaoEditor.prototype.appendWhereLi = function($ul, wheres, table, isPiece) {
		...
		if (wheres.length > 0) {
			if (!isPiece) {
				$li.append('<span class="pdr-10 coos-orange">WHERE 1=1</span>');
			} else {
				$li.append('<span class="pdr-10 coos-orange">1=1</span>');
			}
		}
		var $btn = $('<a class="mgr-10 coos-pointer coos-green">添加条件</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var where = {};
		...
		var $btn = $('<a class="mgr-10 coos-pointer coos-green">添加条件块</a>');
		$li.append($btn);
		$btn.click(function() {
			that.recordHistory();
			var where = {};
		...
		if (table) {
			var $btn = app.design.service.createSelectColumnBtn(table.id, function(c) {
				that.recordHistory();
				var where = {};
				wheres.push(where);
				where.tablealias = 'T1';
...
(function() {
	...
	ServiceEditor.prototype.buildServiceView = function($box, model) {
		...
		var $a = $('<a class="coos-btn coos-green coos-btn-link coos-btn-xs">收起</a>');
		$a.click(function() {
	...
	ServiceEditor.prototype.buildDesign = function() {
		...
		coos.plugin.load("jsplumb", function() {
		...
		$box.on("contextmenu", function(e) {
			...
			var $contextmenu = coos.contextmenu.createMenu(menus);
			that.$contextmenu = $contextmenu;
			$('body').append(that.$contextmenu);
	...
	ServiceEditor.prototype.toSetting = function() {
		...
		$form.append('<input name="requestmapping" input-skin="0" skin-size="12" label="映射路径" class="coos-input" />');
		$form.append('<input name="beforeprocessor" input-skin="0" skin-size="6" label="前置处理器" class="coos-input" />');
		$form.append('<input name="validator" input-skin="0" skin-size="6" label="验证器" class="coos-input" />');
		$form.append('<input name="afterprocessor" input-skin="0" skin-size="6" label="后置处理器" class="coos-input" />');
		$form.append('<input name="resultresolve" input-skin="0" skin-size="12" label="结果报文配置" info="此处默认使用最后节点得到的结果集" after-addon-class="coos-icon-setting setJexlScriptBtn" class="coos-input" />');

		coos.format.init($form);

		coos.form.full($form, model, true);
...
(function() {
	...
	ServiceEditor.prototype.buildProcessForm = function($html, process) {
		var that = this;
		...
		$form.append('<input name="name" required="1" input-skin="0" info="名称可包含英文、数字、（_）、（-）" pattern="^[a-zA-Z0-9_\-]{0,20}$" skin-size="12" label="名称" class="coos-input" />');

		$form.append('<input name="text" required="1" input-skin="0" skin-size="12" label="文案" class="coos-input" />');

		$form.append('<input name="type" required="1" input-skin="0" skin-size="12" label="类型" class="coos-input coos-input-radio" />');
		var $select = $('<select class="coos-option-cache"/>');
		$form.append($select);
		$select.append('<option value="DECISION">决策节点</option>');
		$select.append('<option value="CONDITION">条件节点</option>');
		$select.append('<option value="DAO">数据节点</option>');
		$select.append('<option value="SUB_SERVICE">子服务节点</option>');
		$select.append('<option value="ERROR_END">异常结束节点</option>');

		if (process.type == 'DAO') {
			$form.append('<input name="daoname" required="1" input-skin="0" skin-size="12" label="数据访问名称" class="coos-input coos-input-select" />');
			var $select = $('<select class="coos-option-cache"/>');
			$form.append($select);
			$(this.getBeans('DAO')).each(function(index, one) {
				var text = one.name;
				if (!coos.isEmpty(one.comment)) {
					text += '（' + one.comment + '）';
				}
				$select.append('<option value="' + one.name + '">' + text + '</option>');
			});

			$form.append('<input name="data" input-skin="0" skin-size="12" label="配置数据" info="此处配置传入数据，默认使用$data解析的数据" after-addon-class="coos-icon-setting setJexlScriptBtn" class="coos-input" />');
		} else if (process.type == 'SUB_SERVICE') {
			$form.append('<input name="servicename" required="1" input-skin="0" skin-size="12" label="服务名称" class="coos-input coos-input-select" />');
			var $select = $('<select class="coos-option-cache"/>');
			$form.append($select);
			$(this.getBeans('SERVICE')).each(function(index, one) {
				var text = one.name;
				if (!coos.isEmpty(one.comment)) {
					text += '（' + one.comment + '）';
				}
			...
			$form.append('<input name="condition" required="1" input-skin="0" skin-size="12" label="条件" after-addon-class="coos-icon-setting setJexlScriptBtn" class="coos-input " />');
		} else if (process.type == 'ERROR_END') {
			$form.append('<input name="errcode" required="1" input-skin="0" skin-size="12" label="错误码" class="coos-input " />');
			$form.append('<input name="errmsg" required="1" input-skin="0" skin-size="12" label="错误信息" class="coos-input " />');
		}

		coos.format.init($form);

		coos.form.full($form, process, true);

		$html.find('[name]').change(function() {
			var name = $(this).attr('name');
			process[name] = $(this).val();
			if (name == 'type') {
				that.buildProcessForm($html, process);
			}
		});
	...
	ServiceEditor.prototype.toInsertProcess = function(process) {
		...
		this.buildProcessForm($html, process);
	};
	...
	ServiceEditor.prototype.toDeleteProcess = function(process, eventData) {
		var that = this;
		if (process.type == 'START') {
			coos.box.error('开始节点不能删除！');
			return;
		}
		if (process.type == 'END') {
			coos.box.error('结束节点不能删除！');
			return;
		}
		coos.box.confirm('确定删除' + process.name + '节点？', function() {
			$(that.model.processs).each(function(index, one) {
				if (one.to == one.name) {
					delete one.to;
				}
	...
	ServiceEditor.prototype.buildProcesssView = function($box, processs) {
		...
		jsPlumb.ready(function() {
			...
			$(processs).each(function(index, process) {
				...
				var $ul = $('<div  />');
				var $btn = $('<a class="coos-btn validate-btn coos-btn- coos-btn-xs coos-green" title="验证">验证</a>');
				$btn.click(function(e) {
					that.toViewValidate(process);
				});
				$ul.append($btn)
				var $btn = $('<a class="coos-btn variable-btn coos-btn- coos-btn-xs coos-green" title="变量">变量</a>');
				$btn.click(function(e) {
					that.toViewVariable(process);
				});
				$ul.append($btn)
				var $btn = $('<a class="coos-btn result-btn coos-btn- coos-btn-xs coos-orange" title="结果">结果</a>');
				$btn.click(function(e) {
					that.toViewResult(process);
				});
				$ul.append($btn)
				var $btn = $('<a class="coos-btn coos-btn- coos-btn-xs coos-green" title="设置">设置</a>');
				$btn.click(function(e) {
					that.toUpdateProcess(process);
				});
				$ul.append($btn)
				var $btn = $('<a class="coos-btn coos-btn- coos-btn-xs coos-red" title="删除">删除</a>');
				$btn.click(function(e) {
					that.toDeleteProcess(process);
				});
			...
			design_.makeTarget($processBox.find(".process-node .ring"), {
				...
				beforeDrop : function(params) {
					...
					if (sourceProcess.type == 'END') {
						coos.box.warn('无法从结束节点开始连线！');
						return false;
					}
					if (sourceProcess.type == 'ERROR_END') {
						coos.box.warn('异常结束节点不能连接其他节点！');
						return false;
					}
					if (targetProcess.type == 'START') {
						coos.box.warn('无法连线到开始节点！');
						return false;
					}
					...
					if (sourceProcess.type != 'DECISION') {
						if (sourceProcess.tos.length > 0) {
							coos.box.warn('该节点无法建立多条连线！');
							return false;
						}
					} else {
						if (targetProcess.type != 'CONDITION') {
							coos.box.warn('决策节点只能与条件节点建立连线！');
							return false;
						}
					}
					that.recordHistory();
			...
			design_.bind("click", function(component) {
				coos.box.confirm('是否删除连线？', function() {
...
(function() {
	...
	ServiceEditor.prototype.appendVariables = function($box, process) {
		var that = this;
		$box.append('<h4 class="title coos-orange">变量（名称:值:默认值）</h4>');
		var $list = $('<ul class="coos-grid-box  " />');
		$box.append($list);

		process.variables = process.variables || [];

		$(process.variables).each(function(index, variable) {
			var $li = $('<li class="coos-grid"></li>');
			$list.append($li)
			...
			var $btn = $('<a class="coos-btn coos-btn-xs coos-btn-link coos-green">修改</a>');
			$btn.click(function() {
				that.toUpdateVariable(variable);
			});
			$card.append($btn)

			var $btn = $('<a class="coos-btn coos-btn-xs coos-btn-link coos-red">删除</a>');
			$btn.click(function() {
		...
		var $btn = $('<a class="coos-btn coos-btn-xs coos-btn-link coos-green">添加变量</a>');
		$btn.click(function() {
	...
	ServiceEditor.prototype.buildVariableForm = function($html, variable) {
		var that = this;
		variable = $.extend(true, {}, variable);
		var $form = $('<div class="coos-form coos-input-sm"/>');
		$html.empty();
		$html.append($form);

		$form.append('<input name="name" input-skin="0" skin-size="12" label="名称" class="coos-input" />');

		$form.append('<input name="value" input-skin="0" skin-size="12" label="值" after-addon-class="coos-icon-setting setJexlScriptBtn" class="coos-input" />');

		$form.append('<input name="defaultvalue" input-skin="0" skin-size="12" label="默认值" after-addon-class="coos-icon-setting setJexlScriptBtn" class="coos-input" />');

		$form.append('<input name="valuer" input-skin="0" skin-size="12" label="取值器" class="coos-input" />');


		coos.format.init($form);

		coos.form.full($form, variable, true);

		$html.find('[name]').change(function() {
			var name = $(this).attr('name');
			variable[name] = $(this).val();
		});
	...
	ServiceEditor.prototype.toInsertVariable = function(process) {
		...
		this.buildVariableForm($html, variable);
	};
	...
	ServiceEditor.prototype.toDeleteVariable = function(process, variable) {
		var that = this;
		coos.box.confirm('确定删除该变量？', function() {
			that.recordHistory();
...
(function() {
	...
	ServiceEditor.prototype.appendValidates = function($box, process) {
		var that = this;
		$box.append('<h4 class="title coos-orange">验证（值:必填:类型:表达式:正则）</h4>');
		var $list = $('<ul class="coos-grid-box  " />');
		$box.append($list);
		...
		var $btn = $('<a class="coos-btn coos-btn-xs coos-btn-link coos-green">添加验证</a>');
		$btn.click(function() {
	...
	ServiceEditor.prototype.buildValidateForm = function($html, validate) {
		var that = this;
		validate = $.extend(true, {}, validate);
		var $form = $('<div class="coos-form coos-input-sm"/>');
		$html.empty();
		$html.append($form);
		...
		$form.append('<input name="required" input-skin="0" skin-size="12" label="必填" class="coos-input coos-input-switch" />');

		$form.append('<input name="pattern" input-skin="0" skin-size="12" label="正则" class="coos-input" />');

		$form.append('<input name="rule" input-skin="0" skin-size="12" label="表达式" after-addon-class="coos-icon-setting setJexlScriptBtn" class="coos-input" />');


		$form.append('<input name="type" input-skin="0" skin-size="12" label="类型" class="coos-input coos-input-select" />');
		var $select = $('<select class="coos-option-cache"/>');
		$select.append('<option >请选择</option>');
		$form.append($select);
		$(source.ENUM_MAP.VALUE_TYPE).each(function(index, one) {
			$select.append('<option value="' + one.value + '">' + one.text + '</option>');
		});


		$form.append('<input name="validator" input-skin="0" skin-size="12" label="验证器" class="coos-input" />');

		$form.append('<input name="errcode" input-skin="0" skin-size="12" label="错误码" class="coos-input" />');

		$form.append('<input name="errmsg" input-skin="0" skin-size="12" label="错误信息" class="coos-input" />');


		coos.format.init($form);

		coos.form.full($form, validate, true);
	...
	ServiceEditor.prototype.toInsertValidate = function(process) {
		...
		this.buildValidateForm($html, validate);
	};
	...
	ServiceEditor.prototype.toDeleteValidate = function(process, validate) {
		var that = this;
		coos.box.confirm('确定删除该验证？', function() {
			that.recordHistory();
...
(function() {
	...
	DictionaryEditor.prototype.buildDesign = function() {
		...
		var $box = $('<div class=" pd-5"></div>');
		$design.append($box);

		var model = this.model;
		model.options = model.options || [];

		$(model.options).each(function(index, option) {
			var $html = $('<div class="coos-row col-6 pd-5"/>');
			$box.append($html);
			var $form = $('<div class="coos-form coos-input-sm "/>');
			$form.css('box-shadow', '0px 0px 6px #7d7d7d');
			$html.append($form);

			var $h = $('<h3 class="pdlr-10 coos-orange">选项' + (index + 1) + '</h3>');
			$h.attr('style', 'padding: 0px 0px 5px;border-bottom: 1px solid #ddd;')
			$form.append($h);

			var $btn = $('<a class="coos-btn coos-btn-link coos-btn-xs coos-red float-right">删除</a>');
			$btn.click(function() {
				coos.box.confirm('确定删除该选项？', function() {
					that.recordHistory();
			...
			$h.append($btn);
			var $btn = $('<a class="coos-btn coos-btn-link coos-btn-xs coos-green float-right">保存</a>');
			$btn.click(function() {
			...
			$form.append('<input name="value" input-skin="0" skin-size="4" label="值" class="coos-input" />');
			$form.append('<input name="text" input-skin="0" skin-size="4" label="文案" class="coos-input" />');

			$form.append('<input name="type" input-skin="0" skin-size="4" label="类型" class="coos-input coos-input-select" />');
			var $select = $('<select class="coos-option-cache"/>');
			$select.append('<option value="">使用输入数据</option>');
			$form.append($select);
			$select.append('<option value="TABLE">读Table数据</option>');
			$select.append('<option value="DAO">读Dao数据</option>');
			$select.append('<option value="SERVICE">读Service数据</option>');
			if (option.type == 'TABLE') {
				$form.append('<input name="tablename" input-skin="0" skin-size="3" label="绑定表" class="coos-input" />');
			} else if (option.type == 'DAO') {
				$form.append('<input name="daoname" input-skin="0" skin-size="3" label="绑定Dao" class="coos-input" />');
			} else if (option.type == 'SERVICE') {
				$form.append('<input name="servicename" input-skin="0" skin-size="3" label="绑定服务" class="coos-input" />');
			} else {

			}


			$form.append('<input name="parent" input-skin="0" skin-size="3" label="父" class="coos-input" />');

			$form.append('<input name="fonticon" input-skin="0" skin-size="3" label="字体图标" class="coos-input" />');
			$form.append('<input name="color" input-skin="0" skin-size="3" label="颜色" class="coos-input coos-input-color" />');

			$form.append('<input name="url" input-skin="0" skin-size="4" label="地址" class="coos-input" />');
			$form.append('<input name="html" input-skin="0" skin-size="4" label="HTML" class="coos-input" />');
			$form.append('<input name="image" input-skin="0" skin-size="4" label="图片" class="coos-input coos-input-file-image" />');

			coos.format.init($html);

			coos.form.full($html, option, true);
		...
		var $btn = $('<a class="coos-btn coos-btn-link coos-green">添加选项</a>');
		$btn.click(function() {
			that.recordHistory();
			model.options.push({});
			that.changeModel();
		...
		var $row = $('<div class="coos-row col-12"/>');
		$row.append($btn);
