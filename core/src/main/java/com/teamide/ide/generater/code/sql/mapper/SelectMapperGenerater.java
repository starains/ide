...
public class SelectMapperGenerater extends SqlMapperGenerater {

	protected final Select select;

	public SelectMapperGenerater(String factory_classname, Select select) {
	...
	public StringBuffer generate(int tab) {
		...
		for (StringBuffer selectColumnSql : selectColumnSqls) {
			content.append(getTab(tab)).append(selectColumnSql).append("\n");
		}
		...
		if (select.isForupdate()) {
			...
			content.append("FOR UPDATE").append("\n");
			if (select.isNowait()) {
				content.append(getTab(tab));
				content.append("NOWAIT").append("\n");
			}
		}
		appendUnion(tab);
